/*
Group discrimination testing for Subject System D for Credit dataset
Inputs :
    argv[1] : Train file
*/
#include "header.h"
...
void check_ratio(vector<int> fixed, node* root,vvs tableInfo, string defaultClass ){
   ...
   ofstream fout;
    int option=4;
    string outfile="../../Suites/freshDcredit99.txt";
    fout.open(outfile,ios::out);

    string already = "../../Suites/Dcredit";
    if (option==3 || option==4){
        int fixval=9;//atoi(fix.c_str());
        ifstream fin;
