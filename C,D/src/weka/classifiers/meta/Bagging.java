...
public class Bagging extends RandomizableIteratedSingleClassifierEnhancer 
  ...
  public void buildClassifier(Instances data) throws Exception {
    ...
    //new stuff
    double pred;
    discr_classif = new double [m_Classifiers.length];    
    for (int j = 0; j < m_Classifiers.length; j++) {
        for (int i= 0; i < data.numInstances();i++){
        pred = m_Classifiers[j].classifyInstance(data.instance(i));
        Discrimination.discCalculation(data.instance(i),pred);
        
        }
        discr_classif[j] = Discrimination.getDisc();
        System.out.println(" Disc for "+j+":\t"+ discr_classif[j]);
